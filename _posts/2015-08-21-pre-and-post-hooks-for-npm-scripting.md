...
And now everyone has to have it installed globally? No. This is a _development time_ dependency for my package. It only needs to be installed if you're going to run <code>npm run jslint</code>.
...
Now, while that <code>npm run lint</code> is cool, I still have to remember to run it from time to time. If only I could run it just _before_ testing, failing if the linting didn't pass 100%. What to do? What to do?
