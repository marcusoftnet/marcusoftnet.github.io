...
* The `T` in our case is `WebApplicationFactory<T>` which comes from `Microsoft.AspNetCore.Mvc.Testing` and  is used to create the API in memory.  Pretty nifty, huh - because now we can test the whole thing in memory rather than kicking up a full webserver. Faster feedback for the win!

  * And the `T`  that gets passed to  `WebApplicationFactory<T>`  is our `Startup` class for the API, where all the initialization code resides. The service injection and database seeding for example.
...
When I did that with a ClassFixture I got problems: `System.ArgumentException: An item with the same key has already been added.`. This was my products being added twice. Which is not so surprising when you think about it.
