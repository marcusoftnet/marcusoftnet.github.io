...
- [Part III - where we start to move over to async/await testing](https://www.marcusoft.net/2018/06/refactoring-an-old-koa-or-how-i-learned-a-lot-modern-javascript_iii.html) - this post
...
But there's so much fluffy stuff around this method. See that `co(function * () {` for example? I you remove this then JavaScript all of a sudden doesn't know what `yield` means. See this [blog post for more information](https://www.marcusoft.net/2014/04/WhenIGotGenerators.html). In short: `co` is the thing that helps us to get `yield` to work as an asynchronous processor for us.
