...
1. Ensure that the X-axis date holds the “Closed date”-values
1. For the lead time-series, click to add a label from the A-column, “Slugs”.
1. Now do some customizations
1. First, change the chart title to something like “Lead time chart” or “Running chart”
1. Select the “Lead time” series and set the “Line thickness” to 0 px
1. Select the “Lead time” series and set the “Point size” to 7 px and select a good shape
1. Select the “Lead time” series and select “Custom” which will pick up our Slug. It will display the value when you hover over it. You can remove the data label if you find it cluttered. You can always enable it again later.
1. Select Legend and put the legend at the bottom.
1. You can fiddle around with grid lines for the axis to make it more readable, to your heart's content.
...
I think that is enough for now. We have got started. The next blog posts will be a shorter and only concern a single chart in the dashboard. In [the next post](https://www.marcusoft.net/2019/01/kanbanstats-ii-filter-the-process-chart.html) – let's make a version that allows us to do some filtering.
