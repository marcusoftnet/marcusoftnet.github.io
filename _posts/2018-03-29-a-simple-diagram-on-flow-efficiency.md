---
layout: post
title: "A simple diagram on flow efficiency"
...
The last couple of weeks I have been re-discovering an old diagram (eerm … sketch really). You can find it scribbled on close to every whiteboard in our office now. Because it opened a lot of interesting discussions on flow, flow effieciency and optimisations for value delivery. 
...
The backlog of work is growing much faster than these people are completing work. High stakes, stakeholders and customers getting angrier by the minute, people in the process getting over worked etc. 

The proposed solution; yes, of course. Hire more engineering (step 4/5 in the process) people. I needed a way to visualize that more people in that step might not neccersary help the flow of finished. Because I think that the problem is not in the work but the waiting. 
...
Then draw a line that indicates the flow of the work that you are doing. I've found that in most process that is not yet optimized for flow efficiency it look something like this:

<img src="/img/flowefficiency_2.jpg" width="100%" />

And that is it. 

In all honestly I've stopped here most cases presenting this. Because most people agree with this hypothesis; much more time is spent waiting than actually working. Also this crude diagrams is enough for the rest of the conversations, I've noticed. From this we can easily start and talk about the real problems and what to do about them. 

But you and I can deep dive a bit more. 

### Flow effeciency

The comparision between these two values is called **flow efficiency** and tracks how much of the total lead time is actually work. There's a [great post by Julia Wester](http://www.everydaykanban.com/2016/09/25/flow-efficiency/) that describes this in detail. It's super simple to calculate:

```
flow efficiency = work time / (work time + waiting time)
```

Most processes that is not explicitly optimised for flow would have a flow efficiency of below 15%. Meaning that around 85% of the total lead time is spent waiting.

Now, as I said above I've seldomly had use numbers to convince people that we have a problem in our process. Because this is `exactly how we work` and `yes - I can totally relate to that` in most cases. 
...
```
 ...
 Yup. About normal I would say. And devastingly bad. 89% wasted waiting. 

### Showing the suggested improvement in our digram
...
Let's say that we do. Not only that - it's also hugely succesful and takes away 75% of their working time. What have we achieved then? This: 
...
* It's not - read [Mythical man month](https://www.amazon.com/Mythical-Man-Month-Software-Engineering-Anniversary/dp/0201835959/ref=sr_1_1?ie=UTF8&qid=1522312799&sr=8-1&keywords=Mythical+man+month), written 1984. `Nine women doesn't give birth in a month`
* It is the most expensive thing you can do to solve this problem. More on this below
* It will slow you down even further for awhile, to train and educate the people in the process. 

### Impact of high work in process

In one instance I had a very interesting comment, that stumped me at first:

> That is just ridicoulous! There's never any waiting time like that?! There's always new work to do. 
...
This diagram is not showing buysiness - it's showing flow of value. In fact, through [Littles Law](https://en.wikipedia.org/wiki/Little%27s_law) we can show that this thinking is part of the behaviour. Because the moment we are not working with something we feel obligated to pick up something new to work with. That will make all work in process go slower. 
...
You could also show this problem in the diagram by running your finger across the line. As you reach end a working stage just say `Now this work has to wait, becuase Dep 2 are busy doing other things right now`.  
...
Why is the normal that we have 15% flow efficiency? That is not good. In fact; it is lousy. How can we together have built an organisation that is this stupid? 

Well, that is not hard to understand at all, if you add an equally crude organisation chart below. This will show that we have built the organisation for another purpose. Here's one example: 

<img src="/img/flowefficiency_5.jpg" width="100%" />

The departments are orgnised in silos, but this work requires that we go through all the silos and work together to achieve a result. This is how value is created - it's called the value stream (blogged about [that realisation before - it was painful](http://www.marcusoft.net/2016/04/what-are-you-optimized-for-then.html))

We have an legal department to do legal things, an development department to develop things and sometime even a testing department to test things. They are often tracked on how many things they are doing and rewarded accordingly. The more you test the better. This means that you are always trying to be busy - hence increasing your work in process and probably queue length. 

Now imagine the poor schmuck that comes along and needs your help to complete a task that crosses over many departments; she needs to bargain and hustle a way to make sure that her things gets priority and hence flows faster. 

Naturally work will not flow fast and smooth across department, projects and teams. That is not what the organisation is built for. 
...
It involves a very complicated organisational shift too: 
...
Did you notice how I didn't hired new people, retrained, re-organised (ah well…) or spent any money accomplishing this change? I just let the people that is needed to complete the work… work together to complete the work.
...
This is formalized in software development in a meeting called the [Three Amigos](https://www.agilealliance.org/glossary/three-amigos); where several different disciplinces meet early on to discuss new features and find problems earlier.
...
Better yet, if you dare, lead like it was 1807 and [give them a mission command](https://en.wikipedia.org/wiki/Mission_command), a value you want improved, a WHY and leave the HOW to the group. Ask them; what do you want to measure yourselves on to see improvements.
...
> SO?! We should get the entire company into a room just have everyone waiting then? 

(true quote from upset client)

No. Because your organisation and processes are not yet built to handle that low work in process. Lean companies could be said to have that state as their hypothetical vision - a north star goal. Toyotas "one piece continuous flow" is an example.

But we can't do that. Now. We need to balance the resource efficiency and flow effeciency to what makes sense economically. Or simpler: let's find a suitable number of people, for a suitable number of hours per week that will ensure that this work flows fast and smooth.

Look up the [execellent writings of Dr Donald Reinertsen](https://www.amazon.com/Principles-Product-Development-Flow-Generation/dp/1935401009/ref=sr_1_1?s=books&ie=UTF8&qid=1522314063&sr=1-1&keywords=Principles+of+product+development+flow) on economical models for this. 
...
Ok ok - if you cannot do physical room then do a virtual room instead. Focus more on the `at the same time working on the same thing` part first. Physical room is, in my experience better, but do what you can under the constraints you have. 
