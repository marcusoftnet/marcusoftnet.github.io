...
I have a big project ahead here at [Salt](https://www.salt.dev) - I'm going to write an entire bootcamp from scratch. This is done as a single person project and all my other collegues are working on as normal around me. 

Obviosuly I wanted to report my progress to them, so that they know what's happening and also that I can get some leaway of keeping my focus. 
...
So ... all that is left is to fill out the blank basically. Well, it's not really that simple, but I have to ensure that I managed to fit the content. 

What I did is that I listed all days (and weeks) for the entire bootcamp and then created a status for each day:

1. Documentation initialized
2. Heading of week created
...
The most simple thing I could do was to add a reporting for how all of these numbers add up. This was things like a circle diagram showing how far I've come
...
This is good and probably interesting but actually doesn't say anything about when I will be done. The works I've done so far is the easy part. I could copy just about everything from other sources. 
...
Now, what if I could create a prognosis based of that data. 
...
Each day I progress a few %. Some sessions it's more and other less. Some time I don't get time to work on this for quite some time and others it's a few days after each other. 
...
The formulas uses the `Gain in % per day` kolumn like this
...
I then create a long list of dates, going from the first time I worked on the project and 6 months into the future. For each date I added four columns like below, and incremented the 3 first with the values I calulcated with the percentiles above:
...
That means that the second rows formulas looks like this:
...
Basically take the value from the G2-cell and add it to the value of the row above: `15% + 0.54% = 15.54%`. For the probable and best scenarios I did the same thing, but using those percentiles. 

The final column is the actual value, where I used my original data, that I based the prognosis on. It shows what *actually* happened and makes a nice little addition to the prognosis to see where we are today. 



But I don't have all the dates in that list so I will use the latests value I have until a new is discovered:
...
After filling out all the days six mothns worth of data was easy and I could then create the following graph:

![Projections](/img/DNFSProjections.png)

## Summary and future

The projections are based on the data that I have created so far and hence this graph will be updated as we go. Which exactly what I wanted, because it reflects what has happened. Just like a good weather prognosis we update it as we get more data. 
...
That will make both the acutal line and the projections change into this...

![Projections](/img/DNFSProjections2.png)

And I'm in trouble, but I can explain that I need more time. And I will have a nice way to communicate with my collegues about how things are going and *when I will be done*
